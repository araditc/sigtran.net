...
| SCCP/TCAP/MAP evidence upgrade | Phase 32 in progress: shared protocol evidence vectors, byte-level mismatch validation, SCCP evidence vectors, and TCAP evidence vectors are available; MAP vector suite, trace validation, mismatch correction, and evidence-backed readiness gates are next |
...
Phase 32 upgrades SCCP, TCAP, and MAP SMS toward evidence-backed behavior, with shared byte-level evidence vector validation plus SCCP and TCAP evidence vectors in place before MAP vectors, trace validation, mismatch correction, and evidence-backed readiness gates.
