...
| Stable commercial release gate | Phase 43 in progress: stable release target lock and stable commercial dossier evidence map are available, binding stable package version, source commit, matching `v{version}` tag, retained artifact root, requester identity, UTC target creation time, required retained evidence kinds, SHA-256 digests, and target-root path checks; stable publication remains blocked until readiness checklist, stable decision, protected tag/publish authorization, final report, audit trail, and retained release evidence all pass |
