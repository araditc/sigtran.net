...
| SCCP/TCAP/MAP evidence upgrade | Phase 32 in progress: shared protocol evidence vectors, byte-level mismatch validation, SCCP/TCAP/MAP SMS evidence vectors, cross-layer evidence bundle, ordered trace validation, and actionable mismatch classification are available; evidence-backed readiness gates are next |
...
Phase 32 upgrades SCCP, TCAP, and MAP SMS toward evidence-backed behavior, with shared byte-level evidence vector validation, SCCP/TCAP/MAP SMS evidence vectors, a cross-layer evidence bundle, ordered trace validation, and mismatch classification in place before evidence-backed readiness gates.
