...
| Native SCTP production hardening | Phase 31 in progress: outbound stream/PPID framing, association lifecycle journal, reconnect schedule, send backpressure, cancellation/timeout, and multi-homing readiness contracts are available; production readiness still requires fault recovery and retained Linux SCTP evidence |
...
Phase 31 hardens native SCTP production contracts, starting with outbound stream and PPID framing, association lifecycle journaling, reconnect scheduling, send backpressure, cancellation/timeout policy, and multi-homing readiness before fault recovery hardening.
