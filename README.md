...
| Native SCTP production hardening | Phase 31 in progress: outbound stream/PPID framing, association lifecycle journal, reconnect schedule, send backpressure, and cancellation/timeout contracts are available; production readiness still requires multi-homing, fault recovery, and retained Linux SCTP evidence |
...
Phase 31 hardens native SCTP production contracts, starting with outbound stream and PPID framing, association lifecycle journaling, reconnect scheduling, send backpressure, and cancellation/timeout policy before multi-homing readiness and fault recovery hardening.
