...
| Native SCTP production hardening | Phase 31 in progress: outbound stream/PPID framing, association lifecycle journal, reconnect schedule, and send backpressure contracts are available; production readiness still requires cancellation, multi-homing, fault recovery, and retained Linux SCTP evidence |
...
Phase 31 hardens native SCTP production contracts, starting with outbound stream and PPID framing, association lifecycle journaling, reconnect scheduling, and send backpressure before cancellation, multi-homing readiness, and fault recovery hardening.
