...
| SCCP/TCAP/MAP evidence upgrade | Phase 32 in progress: shared protocol evidence vectors, byte-level mismatch validation, and SCCP evidence vectors are available; TCAP/MAP vector suites, trace validation, mismatch correction, and evidence-backed readiness gates are next |
...
Phase 32 upgrades SCCP, TCAP, and MAP SMS toward evidence-backed behavior, with shared byte-level evidence vector validation and SCCP evidence vectors in place before TCAP/MAP vectors, trace validation, mismatch correction, and evidence-backed readiness gates.
