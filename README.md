...
| Stable commercial release gate | Phase 43 in progress: stable release target lock, stable commercial dossier evidence map, and reviewed readiness checklist are available, binding stable package version, source commit, matching `v{version}` tag, retained artifact root, requester identity, UTC target creation time, required retained evidence kinds, SHA-256 digests, target-root path checks, and approved readiness areas; stable publication remains blocked until stable decision, protected tag/publish authorization, final report, audit trail, and retained release evidence all pass |
