...
| Package publication readiness | Phase 24 in progress: release version policy, NuGet metadata contract, package output layout, dry-run publish plan, credential policy, channel policy, and package integrity manifest added; package publication remains blocked until all publication contracts, credentials, evidence, signing, SBOM, and provenance gates are complete |
