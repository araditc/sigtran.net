...
| Supply-chain release execution | Phase 34 in progress: final versioned SBOM, trusted timestamped package signing, provenance attestation, public API diff, artifact upload, ordered command-plan, release promotion gate, concrete workflow execution, and status contracts are available; final validation and retained release-run artifacts are still required |
