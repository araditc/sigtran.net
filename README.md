...
| Performance and resilience evidence | Phase 33 in progress: peer-traffic benchmark workload evidence now models warmup, sustained, and peak stages; retained artifact manifests and run plans require PCAP, SDK trace, peer logs/config, metrics, latency, resource, resilience, and report artifacts before commercial performance evidence can pass |
...
Phase 33 upgrades performance and resilience evidence, starting with a peer-traffic benchmark workload contract and retained artifact run plan that require warmup, sustained, peak, PCAP, SDK trace, peer logs/configuration, metrics, latency, resource, resilience, and report artifacts before publishable evidence is accepted.
