...
| Commercial closure evidence | External C SCTP peer run, peer-traffic benchmark evidence, internal timestamped RC signing evidence, and protected release workflow dry-run `28289987418` are retained; NuGet prerelease publication is enabled through a guarded unsigned prerelease policy until public author-signing material is approved |
...
7. Keep NuGet publication paused until explicitly authorized; use `publish=false` dry-runs for evidence closure.
