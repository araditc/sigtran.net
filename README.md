...
| SCCP/TCAP/MAP evidence upgrade | Phase 32 in progress: shared protocol evidence vectors, byte-level mismatch validation, and SCCP/TCAP/MAP SMS evidence vectors are available; trace validation, mismatch correction, and evidence-backed readiness gates are next |
...
Phase 32 upgrades SCCP, TCAP, and MAP SMS toward evidence-backed behavior, with shared byte-level evidence vector validation plus SCCP, TCAP, and MAP SMS evidence vectors in place before trace validation, mismatch correction, and evidence-backed readiness gates.
