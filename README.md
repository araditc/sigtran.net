...
| Stable commercial release gate | Phase 43 in progress: stable release target lock is available, binding stable package version, source commit, matching `v{version}` tag, retained artifact root, requester identity, and UTC target creation time; stable publication remains blocked until complete commercial dossier, stable decision, protected tag/publish authorization, final report, and retained release evidence all pass |
