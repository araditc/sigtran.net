...
| Native SCTP support | Phase 8 foundation is available for Linux native SCTP; VM loopback evidence now exists for SDK M3UA traffic over kernel SCTP, while external peer evidence is still required before production support; Windows and macOS remain contract-only |
