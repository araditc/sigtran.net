...
await RunAsync(
    ...
    TrackedSendWaitsForTransportCompletionAsync);
await RunAsync(
    "Caller cancellation before transport invocation remains known not-dispatched",
    CallerCancellationBeforeTransportInvocationIsKnownNotDispatchedAsync);
await RunAsync(
    "Failure after transport invocation remains ambiguous",
    FailureAfterTransportInvocationIsAmbiguousAsync);
await RunAsync(
    "Replacement generation does not carry tracked work",
    ReplacementGenerationDoesNotCarryTrackedWorkAsync);
await RunAsync(
    "Stale tracked work fails closed before transport",
    StaleWorkItemFailsClosedWithoutTransportAsync);

static async Task TrackedSendWaitsForTransportCompletionAsync()
{
...
static async Task CallerCancellationBeforeTransportInvocationIsKnownNotDispatchedAsync()
{
    ControlledM3uaTransport transport = new(blockPayload: true);
    ...
    // Occupy the active session send path with an untracked compatibility send.
    // The second tracked item is admitted but cannot invoke transport yet.
...
static async Task FailureAfterTransportInvocationIsAmbiguousAsync()
{
...
static async Task ReplacementGenerationDoesNotCarryTrackedWorkAsync()
{
    ...
    // the runtime replaces its transport session. The tracked item behind it was
    // admitted under generation 1 but never invoked there.
    await runtime.SendAsync(Transfer(sls: 1)).ConfigureAwait(false);
...
static Task StaleWorkItemFailsClosedWithoutTransportAsync()
{
    ...
    Equal(
        ...
        work.Completion.GetAwaiter().GetResult().Disposition,
        "Generation mismatch must remain known not-dispatched.");
    return Task.CompletedTask;
}

static M3uaRuntime CreateRuntime(
        ...
        if (!pending.TryDequeue(out ControlledM3uaTransport? transport))
        {
            return ValueTask.FromException<M3uaRuntimeSessionLease>(
                new InvalidOperationException(
                    "No synthetic M3UA transport remains for runtime session creation."));
        }
...
static Mtp3TransferMessage Transfer(byte sls = 1) => new(
    new Mtp3ServiceInformationOctet(Mtp3ServiceIndicator.Sccp, networkIndicator: 2),
    new Mtp3RoutingLabel(1234, 4321, sls),
    new byte[] { 0x01, 0x02 },
    routingContext: 100);

static async Task<M3uaAssociationSendException> CaptureAssociationFailureAsync(Task task)
{
    try
    {
...
internal static class RuntimeOutboundTestConstants
{
    internal static readonly TimeSpan Timeout = TimeSpan.FromSeconds(5);
}

internal sealed class ControlledM3uaTransport : ISctpTransport
{
        ...
        CancellationToken ct = default)
    {
        ...
        {
            throw new InvalidOperationException(
                "Synthetic M3UA transport received a short PDU.");
        }

        byte messageClass = payload.Span[2];
        byte messageType = payload.Span[3];
        ...
        {
            // Runtime shutdown/control messages do not require a synthetic peer
            // acknowledgement for these ownership tests.
            return;
        }
        ...
        if (_blockPayload)
        {
            await _payloadRelease.Task.WaitAsync(
                RuntimeOutboundTestConstants.Timeout,
...
internal sealed class SyntheticAssociation : ISctpAssociation
{
...
file static class TimeoutProvider
{
    internal static TimeSpan TestTimeout => RuntimeOutboundTestConstants.Timeout;
}

static TimeSpan TestTimeout => RuntimeOutboundTestConstants.Timeout;
