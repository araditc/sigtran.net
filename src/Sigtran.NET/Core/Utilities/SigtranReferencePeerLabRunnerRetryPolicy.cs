...
public sealed class SigtranReferencePeerLabRunnerRetryRule
{
    ...
    public SigtranReferencePeerLabRunnerRetryRule(
        SigtranReferencePeerLabRunnerFailureKind failureKind,
        bool retryable,
        int maxAttempts,
        TimeSpan delay,
        string description)
    ...
    public SigtranReferencePeerLabRunnerFailureKind FailureKind { get; }
...
public sealed class SigtranReferencePeerLabRunnerRetryEvaluation
{
    ...
    public SigtranReferencePeerLabRunnerRetryEvaluation(
        SigtranReferencePeerLabRunnerFailureReport failureReport,
        int attemptNumber,
        IReadOnlyList<SigtranReferencePeerLabRunnerFailure> retryableFailures,
        IReadOnlyList<SigtranReferencePeerLabRunnerFailure> nonRetryableFailures,
        TimeSpan nextDelay)
    {
    ...
    public IReadOnlyList<SigtranReferencePeerLabRunnerFailure> RetryableFailures { get; }

    /// <summary>The non-retryable failures.</summary>
    public IReadOnlyList<SigtranReferencePeerLabRunnerFailure> NonRetryableFailures { get; }
    ...
    public string RenderMarkdown()
    {
        ...
        builder.AppendLine("# Reference Peer Lab Runner Retry Evaluation");
        builder.AppendLine();
...
public sealed class SigtranReferencePeerLabRunnerRetryPolicy
{
    ...
    /// <summary>Creates a reference peer lab runner retry policy.</summary>
    /// <param name="rules">The retry rules.</param>
    public SigtranReferencePeerLabRunnerRetryPolicy(IReadOnlyList<SigtranReferencePeerLabRunnerRetryRule> rules)
    {
    ...
    public IReadOnlyList<SigtranReferencePeerLabRunnerRetryRule> Rules => _rules.ToArray();

    /// <summary>Returns the retry rule for a failure kind.</summary>
    /// <param name="failureKind">The failure kind.</param>
    /// <returns>The retry rule.</returns>
    public SigtranReferencePeerLabRunnerRetryRule GetRule(SigtranReferencePeerLabRunnerFailureKind failureKind)
    {
    ...
    public SigtranReferencePeerLabRunnerRetryEvaluation Evaluate(
        SigtranReferencePeerLabRunnerFailureReport failureReport,
        int attemptNumber)
    {
        ArgumentNullException.ThrowIfNull(failureReport);

        List<SigtranReferencePeerLabRunnerFailure> retryable = [];
        List<SigtranReferencePeerLabRunnerFailure> nonRetryable = [];
        TimeSpan nextDelay = TimeSpan.Zero;

        foreach (SigtranReferencePeerLabRunnerFailure failure in failureReport.BlockingFailures)
        {
            SigtranReferencePeerLabRunnerRetryRule rule = GetRule(failure.Kind);
    ...
    /// <summary>Creates the default reference peer lab runner retry policy.</summary>
    /// <returns>The default retry policy.</returns>
    public static SigtranReferencePeerLabRunnerRetryPolicy CreateDefault()
    {
        return new(
            [
                new(SigtranReferencePeerLabRunnerFailureKind.Preflight, false, 1, TimeSpan.Zero, "Preflight failures require environment or input correction."),
                new(SigtranReferencePeerLabRunnerFailureKind.CommandExecution, true, 3, TimeSpan.FromSeconds(5), "Command failures may be transient when peer startup or capture timing is involved."),
                new(SigtranReferencePeerLabRunnerFailureKind.ArtifactRetention, false, 1, TimeSpan.Zero, "Missing retained artifacts require runner or storage correction."),
                new(SigtranReferencePeerLabRunnerFailureKind.DigestVerification, false, 1, TimeSpan.Zero, "Digest failures require evidence package regeneration or review."),
                new(SigtranReferencePeerLabRunnerFailureKind.Provenance, false, 1, TimeSpan.Zero, "Provenance failures require source or runner identity correction."),
                new(SigtranReferencePeerLabRunnerFailureKind.Comparison, true, 2, TimeSpan.FromSeconds(10), "Comparison failures may be retried once when peer timing is suspected."),
                new(SigtranReferencePeerLabRunnerFailureKind.RunReport, false, 1, TimeSpan.Zero, "Run report failures require report regeneration or operator review.")
            ]);
    }
}
