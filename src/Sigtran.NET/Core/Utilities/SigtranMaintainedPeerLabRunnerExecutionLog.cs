...
/// <summary>
/// Identifies maintained external peer lab runner execution log event kinds.
/// </summary>
public enum SigtranMaintainedPeerLabRunnerLogEventKind
{
...
/// <summary>
/// Describes one maintained external peer lab runner execution log entry.
/// </summary>
public sealed class SigtranMaintainedPeerLabRunnerExecutionLogEntry
{
    /// <summary>Creates a maintained peer lab runner execution log entry.</summary>
    ...
    public SigtranMaintainedPeerLabRunnerExecutionLogEntry(
        DateTimeOffset timestampUtc,
        SigtranMaintainedPeerLabRunnerLogEventKind kind,
        string message,
        SigtranMaintainedPeerLabCommandKind? commandKind = null,
    ...
    public SigtranMaintainedPeerLabRunnerLogEventKind Kind { get; }

    /// <summary>The event message.</summary>
    public string Message { get; }

    /// <summary>The optional command kind.</summary>
    public SigtranMaintainedPeerLabCommandKind? CommandKind { get; }
...
/// <summary>
/// Describes maintained external peer lab runner execution logs.
/// </summary>
public sealed class SigtranMaintainedPeerLabRunnerExecutionLog
{
    private readonly SigtranMaintainedPeerLabRunnerExecutionLogEntry[] _entries;

    /// <summary>Creates a maintained peer lab runner execution log.</summary>
    /// <param name="runId">The lab run id.</param>
    /// <param name="entries">The log entries.</param>
    public SigtranMaintainedPeerLabRunnerExecutionLog(
        string runId,
        IReadOnlyList<SigtranMaintainedPeerLabRunnerExecutionLogEntry> entries)
    {
    ...
    public IReadOnlyList<SigtranMaintainedPeerLabRunnerExecutionLogEntry> Entries => _entries.ToArray();

    /// <summary>Whether the log contains an error event.</summary>
    public bool HasErrors => _entries.Any(static entry => entry.Kind == SigtranMaintainedPeerLabRunnerLogEventKind.Error);

    /// <summary>Whether the log has runner start and completion events.</summary>
    public bool HasLifecycle => _entries.Any(static entry => entry.Kind == SigtranMaintainedPeerLabRunnerLogEventKind.Started)
        && _entries.Any(static entry => entry.Kind == SigtranMaintainedPeerLabRunnerLogEventKind.Completed);
    ...
    public string RenderJsonLines()
    {
        ...
        foreach (SigtranMaintainedPeerLabRunnerExecutionLogEntry entry in _entries)
        {
    ...
    {
        StringBuilder builder = new();
        builder.AppendLine("# Maintained Peer Lab Runner Execution Log");
        builder.AppendLine();
...
/// <summary>
/// Provides maintained external peer lab runner execution log helpers.
/// </summary>
public static class SigtranMaintainedPeerLabRunnerExecutionLogs
{
    ...
    public static SigtranMaintainedPeerLabRunnerExecutionLog CreatePassing(
        SigtranMaintainedPeerLabRunnerCommandManifest commandManifest,
        DateTimeOffset startedUtc)
    {
        ArgumentNullException.ThrowIfNull(commandManifest);

        List<SigtranMaintainedPeerLabRunnerExecutionLogEntry> entries =
        [
            new(startedUtc, SigtranMaintainedPeerLabRunnerLogEventKind.Started, "Runner started.")
        ];
        ...
        foreach (SigtranMaintainedPeerLabRunnerCommandEntry command in commandManifest.Commands)
        {
            ...
            entries.Add(new(commandStart, SigtranMaintainedPeerLabRunnerLogEventKind.CommandStarted, command.Command.Name, command.Command.Kind));
            entries.Add(new(commandStart.AddMilliseconds(500), SigtranMaintainedPeerLabRunnerLogEventKind.CommandCompleted, command.Command.Name, command.Command.Kind));
        }

        entries.Add(new(startedUtc.AddSeconds(index), SigtranMaintainedPeerLabRunnerLogEventKind.Completed, "Runner completed."));
