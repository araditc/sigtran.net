...
public sealed class SigtranReferencePeerLabEvidenceArtifact
{
    ...
    public SigtranReferencePeerLabEvidenceArtifact(
...
public sealed class SigtranReferencePeerLabEvidenceReport
{
    ...
    /// <summary>Creates a reference peer lab evidence report.</summary>
    ...
    public SigtranReferencePeerLabEvidenceReport(
        SigtranReferencePeerLabArtifactPlan artifactPlan,
        SigtranReferencePeerLabPrerequisiteReport prerequisiteReport,
        SigtranReferencePeerLabConfigurationValidation configurationValidation,
        IReadOnlyList<SigtranReferencePeerLabEvidenceArtifact> artifacts,
    ...
    public SigtranReferencePeerLabPrerequisiteReport PrerequisiteReport { get; }

    /// <summary>The configuration validation report.</summary>
    public SigtranReferencePeerLabConfigurationValidation ConfigurationValidation { get; }

    /// <summary>The retained evidence artifacts.</summary>
    public IReadOnlyList<SigtranReferencePeerLabEvidenceArtifact> Artifacts => _artifacts.ToArray();
    ...
    /// <summary>Whether the reference peer lab evidence is ready for production promotion.</summary>
...
public static class SigtranReferencePeerLabEvidence
{
    ...
    public static IReadOnlyList<SigtranReferencePeerLabEvidenceArtifact> CreateDigestCoveredArtifacts(
        SigtranReferencePeerLabArtifactPlan artifactPlan,
        string sha256)
    {
        ArgumentNullException.ThrowIfNull(artifactPlan);
        ...
        return artifactPlan.Items
            .Select(item => new SigtranReferencePeerLabEvidenceArtifact(item.Kind, item.Path, retained: true, sha256))
            .ToArray();
    }
}
