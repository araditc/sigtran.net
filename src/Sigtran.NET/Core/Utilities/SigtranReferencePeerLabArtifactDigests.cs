...
public sealed class SigtranReferencePeerLabArtifactDigest
{
    ...
    public SigtranReferencePeerLabArtifactDigest(
...
public sealed class SigtranReferencePeerLabArtifactDigestManifest
{
    ...
    /// <summary>Creates a reference peer lab artifact digest manifest.</summary>
    /// <param name="artifactPlan">The artifact plan.</param>
    /// <param name="digests">The artifact digests.</param>
    public SigtranReferencePeerLabArtifactDigestManifest(
        SigtranReferencePeerLabArtifactPlan artifactPlan,
        IReadOnlyList<SigtranReferencePeerLabArtifactDigest> digests)
    {
    ...
    public IReadOnlyList<SigtranReferencePeerLabArtifactDigest> Digests => _digests.ToArray();
    ...
    {
        return _digests
            .Select(static digest => new SigtranReferencePeerLabEvidenceArtifact(digest.Kind, digest.Path, retained: true, digest.Sha256))
            .ToArray();
    }
...
public static class SigtranReferencePeerLabArtifactDigestManifests
{
    ...
    public static SigtranReferencePeerLabArtifactDigestManifest CreateDigestCovered(
        SigtranReferencePeerLabArtifactPlan artifactPlan,
        string sha256)
    {
        ArgumentNullException.ThrowIfNull(artifactPlan);
        ...
        return new(
            artifactPlan,
            artifactPlan.Items
                .Select(item => new SigtranReferencePeerLabArtifactDigest(item.Kind, item.Path, sha256))
                .ToArray());
    }
}
