...
public sealed class SigtranReferencePeerLabConfiguration
{
    ...
    public SigtranReferencePeerLabConfiguration(
        string peerName,
        string localIp,
        int localSctpPort,
        string remoteIp,
    ...
    /// <summary>Validates the reference peer lab configuration.</summary>
    /// <returns>The configuration validation report.</returns>
    public SigtranReferencePeerLabConfigurationValidation Validate()
    {
        List<string> errors = [];
...
public sealed class SigtranReferencePeerLabConfigurationValidation
{
    ...
    public SigtranReferencePeerLabConfigurationValidation(IReadOnlyList<string> errors)
    {
...
public static class SigtranReferencePeerLabConfigurations
{
    ...
    public static SigtranReferencePeerLabConfiguration CreateDefault()
    {
        return new(
            peerName: "reference-external-peer",
            localIp: "127.0.0.1",
            localSctpPort: 2905,
            remoteIp: "127.0.0.1",
            remoteSctpPort: 2906,
            ...
            artifactRoot: "artifacts/external-peer/reference");
    }

    /// <summary>Creates a reference external peer lab configuration from environment values.</summary>
    /// <param name="environment">The environment values.</param>
    /// <returns>The reference external peer lab configuration.</returns>
    public static SigtranReferencePeerLabConfiguration FromEnvironment(IReadOnlyDictionary<string, string> environment)
    {
        ...
        return new(
            ...
            GetOptional(environment, "ARTIFACT_ROOT", "artifacts/external-peer/reference"));
    }
