...
public sealed class SigtranReferencePeerLabRunnerDigestReport
{
    ...
    public SigtranReferencePeerLabRunnerDigestReport(
...
public static class SigtranReferencePeerLabRunnerDigests
{
    ...
    public static SigtranReferencePeerLabRunnerDigestReport Create(
        ...
        IReadOnlyDictionary<string, string> sha256ByPath)
    {
        ...
        List<SigtranReferencePeerLabArtifactDigest> digests = [];
        foreach (SigtranReferencePeerLabRunnerCollectedArtifact artifact in collection.Artifacts.Where(static artifact => artifact.Retained))
        {
        ...
        return new(collection, new SigtranReferencePeerLabArtifactDigestManifest(artifactPlan, digests));
    }
}
