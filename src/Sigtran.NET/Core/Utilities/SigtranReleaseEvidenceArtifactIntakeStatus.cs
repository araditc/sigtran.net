...
public static class SigtranReleaseEvidenceArtifactIntakeStatus
{
    ...
    public const string StatusLabel = "Production Evidence Artifact Intake";

    /// <summary>The number of completed production evidence artifact intake work units.</summary>
    ...
    /// <summary>Whether artifact intake currently allows production publication.</summary>
    public static bool ProductionPublicationReady => ArtifactIntakeFoundationReady
        && RealArtifactEvidenceReady
        && DefaultBlockers.Length == 0;

    /// <summary>Formats a compact production evidence artifact intake summary.</summary>
    /// <returns>The production evidence artifact intake summary.</returns>
    public static string Describe()
    {
        return $"{StatusLabel}: completedUnits={CompletedUnitCount} capabilities={Capabilities.Length} foundationReady={ArtifactIntakeFoundationReady} realArtifactEvidenceReady={RealArtifactEvidenceReady} productionPublicationReady={ProductionPublicationReady} blockers={DefaultBlockers.Length}";
    }

    private static SigtranReleaseEvidenceDossierIntakeBridgeResult CreateDefaultBridgeResult()
    {
        ...
        return SigtranReleaseEvidenceDossierIntakeBridge.BuildDefault(
            run,
            "intake-20260622-001",
            "release-review",
