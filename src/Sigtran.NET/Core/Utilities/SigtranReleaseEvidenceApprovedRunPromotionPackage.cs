...
public sealed class SigtranReleaseEvidenceApprovedRunPromotionArtifact
{
    ...
    public SigtranReleaseEvidenceApprovedRunPromotionArtifact(
        string id,
        string retainedPath,
        string sha256,
        bool required)
...
public sealed class SigtranReleaseEvidenceApprovedRunPromotionPackage
{
    ...
    public SigtranReleaseEvidenceApprovedRunPromotionPackage(
        string packageId,
        SigtranReleaseEvidenceRunApprovalReportWriteResult approvalReport,
        IReadOnlyList<SigtranReleaseEvidenceApprovedRunPromotionArtifact> artifacts,
    ...
    public SigtranReleaseEvidenceRunApprovalReportWriteResult ApprovalReport { get; }

    /// <summary>The promotion package artifacts.</summary>
    public IReadOnlyList<SigtranReleaseEvidenceApprovedRunPromotionArtifact> Artifacts { get; }
    ...
    {
        return $"productionEvidenceApprovedRunPromotionPackageReady={IsReady} packageId={PackageId} artifacts={Artifacts.Count}";
    }
}
...
public static class SigtranReleaseEvidenceApprovedRunPromotionPackages
{
    ...
    public static SigtranReleaseEvidenceApprovedRunPromotionPackage CreateDefault(
        SigtranReleaseEvidenceRunApprovalReportWriteResult approvalReport,
        DateTimeOffset createdAtUtc)
    {
        ArgumentNullException.ThrowIfNull(approvalReport);
        SigtranReleaseEvidenceApprovedRunTarget target = approvalReport.Manifest.Checklist.Target;
        SigtranReleaseEvidenceFileSystemPromotionExecution promotion = target.PromotionExecution;
    ...
    private static string ComputePublicationAttachmentDigest(SigtranReleaseEvidencePublicationAttachmentManifest manifest)
    {
        StringBuilder builder = new();
        builder.Append(manifest.Seal.SealId).Append('|').Append(manifest.Seal.AggregateSha256).Append('\n');

        foreach (SigtranReleaseEvidencePublicationAttachment attachment in manifest.Attachments
    ...
    private static string ComputePromotionGateDigest(SigtranReleaseEvidenceFileSystemPromotionExecution promotion)
    {
        StringBuilder builder = new();
