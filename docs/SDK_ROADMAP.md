...
The current repository has moved beyond proof-of-concept status into a foundation-complete SDK with governed release contracts, protocol evidence vectors, native SCTP readiness contracts, retained external C SCTP peer evidence, retained peer-traffic benchmark evidence, retained internal timestamped RC signing evidence, and a passing protected release workflow dry-run. Stable commercial publication is still blocked until approved package publication and NuGet publication evidence are complete.
...
Status: Phase 25 has execution evidence in place. Linux SCTP loopback evidence is retained from a real Ubuntu 22.04 VM, external C SCTP peer evidence is retained for the RC gate, peer-traffic benchmark evidence is retained, internal timestamped RC signing evidence is retained, and protected release workflow dry-run `28289987418` passed with artifact upload and `publish=false`. The legacy OpenSS7/IPSS7 attempt remains retained blocker evidence for Linux 5.15 `open_softirq` compatibility. Stable commercial publication remains blocked on explicit package publication approval, public/stable signing policy execution, and NuGet publication evidence.
